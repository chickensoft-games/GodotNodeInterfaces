...
Godot node interfaces.

Interfaces won't have documentation until we figure out a workaround for <https://github.com/dotnet/roslyn/issues/23673>.
...
dotnet new --install Chickensoft.GodotNodeInterfaces
...
You can debug the included test project for your package in `Chickensoft.GodotNodeInterfaces.Tests/` by opening the root of this repository in VSCode and selecting one of the launch configurations: `Debug Tests` or `Debug Current Test`.
...
By default, a test project in `Chickensoft.GodotNodeInterfaces.Tests/` is created for you to write tests for your package. [GoDotTest] is already included and setup, allowing you to focus on development and testing.
...
# Run code coverage:
cd Chickensoft.GodotNodeInterfaces.Tests
./coverage.sh
```
...
Tests are executed by running the Godot test project in `Chickensoft.GodotNodeInterfaces.Tests` from the command line and passing in the relevant arguments to Godot so that [GoDotTest] can discover and run tests.
...
[line-coverage]: Chickensoft.GodotNodeInterfaces.Tests/badges/line_coverage.svg
[branch-coverage]: Chickensoft.GodotNodeInterfaces.Tests/badges/branch_coverage.svg
