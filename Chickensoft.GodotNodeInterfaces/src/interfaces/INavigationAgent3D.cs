...
public interface INavigationAgent3D {
    ...
    /// <summary>
    /// <para>Returns the <see cref="Rid" /> of the navigation map for this NavigationAgent node. This function returns always the map set on the NavigationAgent node and not the map of the abstract agent on the NavigationServer. If the agent map is changed directly with the NavigationServer API the NavigationAgent node will not be aware of the map change. Use <see cref="M:Godot.NavigationAgent3D.SetNavigationMap(Godot.Rid)" /> to change the navigation map for the NavigationAgent and also update the agent on the NavigationServer.</para>
    /// </summary>
    ...
    /// <summary>
    /// <para>Sets the new wanted velocity for the agent. The avoidance simulation will try to fulfill this velocity if possible but will modify it to avoid collision with other agents and obstacles. When an agent is teleported to a new position, use <see cref="M:Godot.NavigationAgent3D.SetVelocityForced(Godot.Vector3)" /> as well to reset the internal simulation velocity.</para>
    /// </summary>
