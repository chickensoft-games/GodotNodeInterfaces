...
/// </summary>
public interface IAnimatedSprite3D : ISpriteBase3D {
    /// <summary>
    ...
    bool IsPlaying();
    /// <summary>
    /// <para>Plays the animation with key <paramref name="name" />. If <paramref name="customSpeed" /> is negative and <paramref name="fromEnd" /> is <c>true</c>, the animation will play backwards (which is equivalent to calling <see cref="M:Godot.AnimatedSprite3D.PlayBackwards(Godot.StringName)" />).</para>
    ...
    /// <para>This method is a shorthand for <see cref="M:Godot.AnimatedSprite3D.Play(Godot.StringName,System.Single,System.Boolean)" /> with <c>custom_speed = -1.0</c> and <c>from_end = true</c>, so see its description for more information.</para>
    /// </summary>
    void PlayBackwards(StringName name);
    /// <summary>
    /// <para>Pauses the currently playing animation. The <see cref="AnimatedSprite3D.Frame" /> and <see cref="AnimatedSprite3D.FrameProgress" /> will be kept and calling <see cref="M:Godot.AnimatedSprite3D.Play(Godot.StringName,System.Single,System.Boolean)" /> or <see cref="M:Godot.AnimatedSprite3D.PlayBackwards(Godot.StringName)" /> without arguments will resume the animation from the current playback position.</para>
    ...
    /// <summary>
    /// <para>Returns the actual playing speed of current animation or <c>0</c> if not playing. This speed is the <see cref="AnimatedSprite3D.SpeedScale" /> property multiplied by <c>custom_speed</c> argument specified when calling the <see cref="M:Godot.AnimatedSprite3D.Play(Godot.StringName,System.Single,System.Boolean)" /> method.</para>
    ...
    /// <summary>
    /// <para>The displayed animation frame's index. Setting this property also resets <see cref="AnimatedSprite3D.FrameProgress" />. If this is not desired, use <see cref="M:Godot.AnimatedSprite3D.SetFrameAndProgress(System.Int32,System.Single)" />.</para>
    /// </summary>
    int Frame { get; set; }
    /// <summary>
